...
바뀐 URL 은 두 워크플로가 `scripts/indexnow.py` 로 IndexNow(네이버·빙 등)에 자동 제출합니다
(오늘 공개된 글, 금리가 실제로 바뀐 날의 비교 페이지, 수정한 글·페이지).
구글은 IndexNow 에 참여하지 않으므로 사이트맵 크롤링에 맡기고, 급하면 서치콘솔에서 수동 요청합니다.
`scripts/gsc_index_check.py` 는 최근 글·수정 페이지의 구글 색인 상태를 URL 검사 API 로 조회해
"색인 요청"·"재크롤링 요청"이 필요한 URL 목록을 만듭니다(매주 월요일 아침 Claude 루틴이 실행해 보고).
색인 생성 요청 자체는 API 가 없어 사람이 서치콘솔에서 눌러야 합니다.
`sync-to-pages` 워크플로를 수동 실행(workflow_dispatch)하면서 `urls` 에 경로를 넣으면 지정 URL 만 제출할 수 있습니다.
